...
const ListaDePresentes = () => {
  ...
  const gerar_pix_copiaecola = (
    ...
    payload += format_field(
      "26",
      format_field("00", "br.gov.bcb.pix") + format_field("01", chave_pix)
    );
    ...
    <main>
      ...
      <section id="conteudo-principal">
        <FotoTexto titulo="A Lista de Presentes">
          <p>
            Devido aos noivos já possuírem a maioria dos itens necessários para
            a casa, optamos por uma lista de presentes virtual. Esses presentes
            são simbólicos e representam uma contribuição para a nossa lua de
            mel e vida a dois.
          </p>
          <p>
            Os itens possuem cunho cômico e valores aproximados, para que você
            possa escolher o que mais lhe agrada. A contribuição é feita por
            PIX, onde você pode escolher o presente que deseja nos dar e fazer a
            contribuição diretamente.
          </p>
          <p>
            Ao selecionar o presente, você será redirecionado ao código PIX e QR
            Code para efetuar a contribuição.
          </p>
          <p>
        ...
        <div className="presentes">
          {presentes.map((presente, index) => (
            <div className="presente" key={index}>
              <img
                src={`${process.env.PUBLIC_URL}/assets/presentes/${presente.foto}`}
                alt={presente.nome}
              />
              <h3>{presente.nome}</h3>
